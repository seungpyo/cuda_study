...
int ipcOpenAndBindSocket(struct sockaddr_un * local_addr) {
    ...
    if (name_len >= sizeof(local_addr->sun_path)) {
        panic("MemMapManager: Name is too long");
    }
    strncpy(local_addr->sun_path, MemMapManager::endpointName, name_len);

    if (bind(sock_fd, (struct sockaddr *)local_addr, SUN_LEN(local_addr)) < 0) {
        std::cout << getpid() << " local_addr: " << local_addr->sun_path << std::endl;
